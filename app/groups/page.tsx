...
export default function GroupChatsPage() {
  ...
  // Poll for messages in the selected group chat
  useEffect(() => {
    if (!selectedChat) return;

    // Fetch messages immediately when chat is selected
    ...
    <div className="flex h-screen">
      ...
      <div className="w-64 sm:w-80 flex flex-col" style={{ backgroundColor: 'var(--bg-secondary)', borderRight: '1px solid var(--border-color)' }}>
        <div className="p-5" style={{ borderBottom: '1px solid var(--border-color)', backgroundColor: 'var(--bg-tertiary)' }}>
          <h1 className="text-xl font-bold mb-4" style={{ color: 'var(--text-primary)' }}>Groups</h1>
          <button
            onClick={() => setShowCreateForm(!showCreateForm)}
            className="w-full px-5 py-2.5 text-white text-sm font-medium rounded-md transition"
        ...
        {showCreateForm && (
          <form onSubmit={handleCreateGroup} className="p-5 space-y-4" style={{ borderBottom: '1px solid var(--border-color)', backgroundColor: 'var(--bg-tertiary)' }}>
            <input
              type="text"
            ...
            <button
              type="submit"
              className="w-full px-4 py-2.5 text-white text-sm font-medium rounded-md transition"
      ...
      <div className="flex-1 flex flex-col">
        {selectedChat ? (
          <>
            {/* Chat Header */}
            <div className="p-5" style={{ borderBottom: '1px solid var(--border-color)', backgroundColor: 'var(--bg-secondary)' }}>
